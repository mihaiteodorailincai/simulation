...
def sample_zone_nodes(G_walk, hub_nodes_df, n_samples_per_zone=200):
    """
    Sample random delivery points within each zone's walking network.
    Increased samples for realistic scale.
    """
    zone_delivery_nodes = {}
...
def generate_realistic_scale_demand(G_walk, hub_nodes_df, output_path="../data/raw/demand_synthetic.csv"):
    """
    Generate synthetic demand with REALISTIC SCALE (3,585 packages/day) and peak patterns.
    """
    
    # 1. Sample delivery points for each zone (more for realistic scale)
    zone_delivery_nodes = sample_zone_nodes(G_walk, hub_nodes_df, n_samples_per_zone=200)
    
    # 2. REALISTIC SCALE demand parameters (10% Eindhoven penetration)
    daily_volumes = {
        "Centrum": 1613,      # 45% of 3,585
        "Blixembosch": 1254,  # 35% of 3,585  
        "Meerhoven": 716      # 20% of 3,585
    }
    
    print(f"REALISTIC SCALE DEMAND GENERATION")
    print(f"Total daily packages: {sum(daily_volumes.values()):,}")
    print(f"This represents 10% penetration of Eindhoven delivery market")
    ...
    print(f"File size: {len(demand_df.to_csv(index=False)) / 1024 / 1024:.1f} MB")
...
if __name__ == "__main__":
    ...
    print("Generating REALISTIC SCALE demand with peak patterns...")
    print("="*60)
    demand_df = generate_realistic_scale_demand(G_walk, hub_nodes_df)
    print("Done!")